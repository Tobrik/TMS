...
import { DISEASE_LABELS } from "@/lib/diseaseWeights";
...
export default function DoctorPage() {
  ...
  const searchPatientById = async (pid: number) => {
    ...
    try {
      ...
      if (infoResp.found && infoResp.patient) {
        setPatientInfo(infoResp.patient);
      } else {
  ...
  // Get active symptoms for a day (value > 0)
  function getActiveSymptoms(dayId: number): { code: string; label: string; value: number }[] {
    ...
    for (const [code, value] of Object.entries(syms)) {
      if (value > 0) {
        result.push({
          code,
          label: SYMPTOM_LABELS[code as SymptomCode] || code,
          value,
        });
      }
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-md space-y-6">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">TMS - Врач</h1>
            <p className="text-sm text-gray-500">Портал для врачей</p>
          </div>
          ...
          <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
            ...
            <CardContent className="pt-4 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">Выберите врача</label>
                {doctorsLoading ? (
              ...
              <Button
                className="w-full bg-gradient-to-r from-indigo-600 to-purple-500 hover:from-indigo-700 hover:to-purple-600 text-white shadow-lg shadow-indigo-200"
                onClick={handleLogin}
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-gray-900">Поиск пациента</h2>
            </div>
            <div className="flex gap-2">
              <Input
                type="text"
                placeholder="Код пациента (например: OWL-3 или 3)"
        ...
        {triagePatients.length > 0 && !searchedPatientId && (
          <div className="space-y-4">
            {/* Red Zone */}
            {triagePatients.filter((p) => p.zone === "red").length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <AlertTriangle className="h-5 w-5 text-red-500" />
                  <h2 className="font-semibold text-red-700">Красная зона — Срочно</h2>
                  <Badge className="bg-red-100 text-red-700 border-0 animate-pulse">
                    {triagePatients.filter((p) => p.zone === "red").length}
                  </Badge>
                </div>
                <div className="space-y-2">
                  {triagePatients
                    .filter((p) => p.zone === "red")
                    .map((p) => (
                      <Card key={p.patient_id} className="border-l-4 border-l-red-500">
                        <CardContent className="py-3 px-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="font-medium text-gray-900">{p.full_name}</p>
                              <p className="text-xs text-gray-500">
                                {makeCode(p.patient_id)} {p.city && `· ${p.city}`}
                              </p>
                              {p.last_disease && (
                                <p className="text-sm text-red-600 mt-1">
                                  {DISEASE_LABELS[p.last_disease.split(" ")[0]] || p.last_disease.split(" ")[0]}
                            ...
                            <Button
                              ...
                              onClick={() => searchPatientById(p.patient_id)}
                            >
                              Подробнее
                            </Button>
                          </div>
            ...
            {/* Yellow Zone */}
            {triagePatients.filter((p) => p.zone === "yellow").length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Clock className="h-5 w-5 text-amber-500" />
                  <h2 className="font-semibold text-amber-700">Жёлтая зона — Сегодня</h2>
                  <Badge className="bg-amber-100 text-amber-700 border-0">
                    {triagePatients.filter((p) => p.zone === "yellow").length}
                  </Badge>
                </div>
                <div className="space-y-2">
                  {triagePatients
                    .filter((p) => p.zone === "yellow")
                    .map((p) => (
                      <Card key={p.patient_id} className="border-l-4 border-l-amber-400">
                        <CardContent className="py-3 px-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="font-medium text-gray-900">{p.full_name}</p>
            ...
            {/* Green Zone */}
            {triagePatients.filter((p) => p.zone === "green").length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <CheckCircle className="h-5 w-5 text-emerald-500" />
                  <h2 className="font-semibold text-emerald-700">Зелёная зона — Плановые</h2>
                  <Badge className="bg-emerald-100 text-emerald-700 border-0">
                    {triagePatients.filter((p) => p.zone === "green").length}
                  </Badge>
                </div>
                <div className="space-y-2">
                  {triagePatients
                    .filter((p) => p.zone === "green")
                    .map((p) => (
                      <Card key={p.patient_id} className="border-l-4 border-l-emerald-400">
                        <CardContent className="py-3 px-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="font-medium text-gray-900">{p.full_name}</p>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant="secondary" className="text-xs font-mono">
                      {makeCode(patientInfo.patient_id)}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      Регистрация: {patientInfo.created_at?.split(" ")[0] || patientInfo.created_at}
                    </span>
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <Badge variant="secondary" className="text-xs">
                      {history.length} записей в дневнике
                    </Badge>
                    {history.length > 0 && (
                      <Button
                        size="sm"
        ...
        {searchedPatientId !== null && patientInfo && (
          <Card>
            <CardContent className="pt-5 pb-4">
              <SymptomTrend patientId={searchedPatientId} />
            </CardContent>
          </Card>
        )}
        ...
        {searchedPatientId !== null && patientInfo && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-gray-900">История диагнозов</h2>
            </div>
            ...
            {historyLoading ? (
              ...
              history.map((entry) => {
                ...
                const diseaseLabel =
                  DISEASE_LABELS[entry.disease_predict] || entry.disease_predict;
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {entry.doctor_name && (
                          <Badge variant="outline" className="text-xs">
                            {entry.doctor_name.split(" ").slice(0, 2).join(" ")}
                          </Badge>
                        )}
                        {isExpanded ? (
                      ...
                      <div className="border-t border-gray-100 px-4 py-4 space-y-4 bg-gray-50/50">
                          ...
                          {isLoadingSymptoms ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                              {activeSymptoms.map((s) => (
                                <span
                                  key={s.code}
                                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border ${severityColor(s.value)}`}
                                >
                                  {s.label}
                                  <span className="opacity-70">({s.value}/3)</span>
                                </span>
                              ))}
                            </div>
                          ) : (
                            <p className="text-xs text-gray-400 italic">Нет данных о симптомах</p>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                            Предварительный диагноз (ML)
                          </p>
                          <p className="text-sm text-gray-900">{diseaseLabel}</p>
                        </div>

                        {/* AI Clinical Analysis for Doctor */}
                        {entry.doctor_explanation && (
                          <div className="space-y-2 bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                            <p className="text-xs font-medium text-indigo-600 uppercase tracking-wide flex items-center gap-1.5">
                              ...
                              AI-Ассистент: Клиническое обоснование
                            </p>
                            ...
                            <p className="text-xs text-indigo-400 italic mt-2">
                              Данный анализ сгенерирован AI и является вспомогательным инструментом.
                        ...
                        <div className="space-y-1">
                          ...
                          {isEditing ? (
                            <Input
                              ...
                              placeholder="Установить диагноз..."
                              className="bg-white"
                            />
                          ) : (
                            <p className="text-sm text-gray-900">
                              {entry.disease_setup && entry.disease_setup !== "Nothing" ? (
                                ...
                                <span className="text-gray-400 italic">Не установлен</span>
                              )}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          {isEditing ? (
                            <Textarea
                              value={editRecept}
                              onChange={(e) => setEditRecept(e.target.value)}
                              placeholder="Написать рецепт..."
                              rows={3}
                              className="bg-white"
                            />
                          ) : (
